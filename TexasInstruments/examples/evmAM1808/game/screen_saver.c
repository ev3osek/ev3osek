...
ScreenSaver(void)
{
	...
	while(1)
	{
		// Configure to LED mode and toggle LED.
		I2C0IntRegister(4);
		LedIfConfig();
		LedToggle();
		
        // Configure to TS mode and check if TS is touched.
		ulLoop = 50;
	...
	// Configure to LED mode
	I2C0IntRegister(4);
	LedIfConfig();
	LedOff(); // Update the LED to OFF state
		
	// Configure to TS mode
	I2C0IfConfig(I2C_SLAVE_PMIC_ADDR, 50000);
